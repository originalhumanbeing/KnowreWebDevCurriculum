...
* 인터넷을 구성하는 여러 가지 프로토콜
  ...
  * TCP
   - TCP/IP: 계층형 프로토콜
   - 네트워크 인터페이스 (케이블과 같은 물리적인 부분) -> 인터넷 계층 (IP: 데이터 전송을 담당) -> 트랜스포트 계층 (TCP: 커넥션을 연결하고 데이터의 누락이 없는지 체크하여 데이터의 무결성을 보장하는 역할) -> 애플리케이션 계층 (HTTP/SSH 등)으로 구성.
  * HTTP
   - 요청/응답형 프로토콜, 동기형 프로토콜
   - 요청 메시지: 요청 라인, 요청 헤더, 요청 바디로 구성
   - 응답 메시지: 스테이터스 라인, 응답 헤더, 응답 바디로 구성
* DNS
